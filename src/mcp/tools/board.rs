...
impl Amaranth {
    ...
    #[tool(
        description = "게시글 1건의 본문(평문)·댓글을 조회한다. 본문 삽입 이미지는 평문에 `[이미지]`로 자리만 남고, 첫 장의 경로는 `imgPath`로 낸다 — **이미지는 정식 첨부가 아니라 `fileCnt`에 안 잡히므로**, fileCnt=0이어도 본문에 이미지가 있을 수 있다. ⚠️ 호출 시 조회수 증가(실제 열람 처리)."
    )]
